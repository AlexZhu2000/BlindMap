...
def setup_train(hypes):
    ...
    try:
        hypes_communication = hypes['model']['args']['fusion_backbone']['communication']
    except (KeyError, TypeError):
        # 如果任何一层键不存在，返回一个默认的空配置
        hypes_communication = {
            'use_threshold': None,
            'thre': None,
            'comm_volume_MB': None
        }
    if hypes_communication['use_threshold'] is not None:
        if hypes_communication['use_threshold']:
            thre = hypes_communication['thre']
            folder_name = folder_name + f'_thre_{thre}'
        else:
            volume = hypes_communication['comm_volume_MB']
            folder_name = folder_name + f'_comm_volumeMB_{volume}'
    else:
        if hypes_communication['comm_volume_MB'] is not None:
            volume = hypes_communication['comm_volume_MB']
            folder_name = folder_name + f'_comm_volumeMB_{volume}'
        else:
            None
    if hypes['noise_setting'] is not None:
        if hypes['noise_setting']['add_noise']:
            folder_name += '_add_noise'
    if hypes['use_history'] is not None:
        if hypes['use_history']:
            folder_name += '_use_history'
...
def create_blindmap_loss(hypes):
    '''
    Create the loss function based on the given loss name.
    '''
    loss_func_name = hypes['blindmap_loss']['core_method']
    loss_func_config = hypes['blindmap_loss']['args']
    loss_filename = "opencood.loss." + loss_func_name
    loss_lib = importlib.import_module(loss_filename)
    loss_func = None
    target_loss_name = loss_func_name.replace('_', '')
