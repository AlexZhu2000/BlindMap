...
class ZZH_IntermediateFusionDatasetDAIR(Dataset):
    """
    This class is for intermediate fusion where each vehicle transmit the
    deep features to ego.
    """
    def __init__(self, params, visualize, train=True):
        print('ZZH_IntermediateFusionDatasetDAIR')
        self.params = params
    ...
    def retrieve_base_data(self, idx):
        ...
        data : dict
            ...
            each cav.
        """
        # print('test retrieve_base_data...zzh')
        ...
        transformation_matrix = veh_side_rot_and_trans_to_trasnformation_matrix(lidar_to_novatel_json_file,novatel_to_world_json_file) #4X4 matrix
        ...
        ####################zzh######################
        # print('cooperative_vehicle_label vs vehicle_side_label(num):', len(data[0]['params']['vehicles']), 'vs', len(data[0]['params']['vehicles_single']))
        # print('cooperative_vehicle_label:', [label['type'] for label in data[0]['params']['vehicles']] )
        # print('vehicle_side_label:', [label['type'] for label in data[0]['params']['vehicles_single']])
        ####################zzh######################
    ...
    def some_test(self):
        print('zzh_datasset test....')
        co_datainfo = load_json(os.path.join(self.root_dir, 'cooperative/data_info.json'))
        self.co_data = OrderedDict()
        for frame_info in co_datainfo:
            frame_info['cooperative_label_path'] = frame_info['cooperative_label_path'].replace('label_world','label_world_backup')
            veh_frame_id = frame_info['vehicle_image_path'].split("/")[-1].replace(".jpg", "")
            self.co_data[veh_frame_id] = frame_info
        base_data_dict = self.retrieve_base_data(0)
        ego_data_dict = base_data_dict[0]
        infra_data_dict = base_data_dict[1]

        ego_pose = ego_data_dict['params']['lidar_pose']
        #############for ego vehicle##################
        ego_lidar_np = ego_data_dict['lidar_np']
        ego_lidar_np = mask_points_by_range(ego_lidar_np,
                                        self.params['preprocess'][
                                            'cav_lidar_range'])
        # pcd_utils.visualize_point_cloud_open3d(ego_lidar_np)
        # pcd_utils.visualize_point_cloud_mlab(ego_lidar_np)
        processed_lidar = self.pre_processor.preprocess(ego_lidar_np)
        for key, value in processed_lidar.items():
        
            print(key, value.shape)
        print('coords:', processed_lidar['voxel_coords'][0:10]) 


        #############for infra side##################
        infra_lidar_np = infra_data_dict['lidar_np']
        transformation_matrix = \
            x1_to_x2(infra_data_dict['params']['lidar_pose'],
                     ego_pose) # T_ego_cav
        projected_lidar = \
            box_utils.project_points_by_matrix_torch(infra_lidar_np[:, :3],
                                                        transformation_matrix)

        infra_lidar_np[:, :3] = projected_lidar
        # pcd_utils.visualize_point_cloud_open3d(infra_lidar_np)
        # pcd_utils.visualize_point_cloud_mlab(infra_lidar_np)
    ...
    def __getitem__(self, idx):
        ...
        # print('cav_id_list(number):', len(cav_id_list))
        ...
        # cooperative_label和vehicle_side_label中的目标顺序是一致的，且len(cooperative_label) > len(vehicle_side_label)
        # 所以前文对于每个json文件中的每个label按照顺序赋予id，然后按照id剔除重复的label。
