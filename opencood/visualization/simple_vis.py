...
def visualize_blindmap(infer_result, pcd, pc_range, save_path, method='3d', left_hand=False):
        ...
        if method == 'bev':
            ...
            if batch_blind_maps is not None:
                ...
                canvas.draw_canvas_points(canvas_xy[valid_mask])
                if gt_box_tensor is not None:
                    canvas.draw_boxes(gt_box_np, colors=(0,255,0), texts=['']*len(gt_name), box_line_thickness=4)
                if pred_box_tensor is not None:
                    canvas.draw_boxes(pred_box_np, colors=(255,0,0), texts=['']*len(pred_name), box_line_thickness=4)
                ...
                if agent_modality_list is not None:
                    ...
                    for i, modality_name in enumerate(agent_modality_list):
                        if i == 0:
                            ...
                            color = (0,191,255) if modality_name == "m1" else \
                                    (255,185,15) if modality_name == "m2" else \
                                    (138,211,222) if modality_name == "m3" else \
                                    (32, 60, 160) if modality_name == "m4" else \
                                    (66,66,66)
                            text = f'agent_{i}'
                        canvas.draw_boxes(cav_box_np[i:i+1], colors=color, texts=[text])
                
                axs[0,0].imshow(canvas.canvas)
                axs[0,0].set_title('Detection Results')
                axs[0,0].axis('off')
