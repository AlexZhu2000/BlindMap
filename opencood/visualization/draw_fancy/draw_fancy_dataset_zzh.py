...
class ZZHSimpleDataset(Dataset):
    ...
    def get_agent_timestamp(self, scenario_path, agent_id, timestamp):
        """
        Given the cav id, return the corresponding timestamp.
       ...
       # first load all paths of different scenarios
        ...
        scenario_index = None
        idx = scenario_folders.index(os.path.join(root_dir,scenario_id))
        scenario_database = self.scenario_database[idx]
        data = OrderedDict()
        # 加载所有智能体在该时间戳的数据
        for cav_id, cav_content in scenario_database.items():
            if timestamp in cav_content:
                data[cav_id] = OrderedDict()
                data[cav_id]['ego'] = cav_content['ego']
