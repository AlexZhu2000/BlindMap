...
def visualize_point_cloud_open3d(pcd_np_points):
    # 提取 x, y, z 坐标
    points = pcd_np_points[:, :3]

    # 创建 Open3D 点云对象
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(points)

    # 如果需要使用 intensity 来设置点云的颜色
    intensity = pcd_np_points[:, 3]  # intensity 范围是 [0, 1]
    colors = plt.cm.jet(intensity / max(intensity))[:, :3]  # 使用 jet colormap
    pcd.colors = o3d.utility.Vector3dVector(colors)

    # 可视化点云
    vis = o3d.visualization.draw_plotly([pcd])
    ...
    fig.update_layout(
        scene=dict(
            xaxis=dict(
                range=[-100.8, 100.8],  # 设置 x 轴的范围
                title='X Axis',
                showgrid=True,  # 显示网格
                showticklabels=True,  # 显示刻度标签
            ),
            yaxis=dict(
                range=[-40, 40],  # 设置 y 轴的范围
                title='Y Axis',
                showgrid=True,
                showticklabels=True,
            ),
...
def visualize_point_cloud_mlab(pcd_np_points):
