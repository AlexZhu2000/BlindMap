...
def calculate_ap(result_stat, iou):
    ...
    for idx, val in enumerate(tp):
        tp[idx] += cumsum
        cumsum += val
    # 输入 fp = [0, 1, 0, 1] → 输出 fp = [0, 1, 1, 2]。
    # 输入 tp = [1, 0, 1, 0] → 输出 tp = [1, 1, 2, 2]。
    rec = tp[:]
    for idx, val in enumerate(tp):
        rec[idx] = float(tp[idx]) / gt_total
    # 假设 tp = [1, 1, 2, 2]，gt_total = 4，则
    # rec = [1/4, 1/4, 2/4, 2/4] = [0.25, 0.25, 0.5, 0.5]。
    prec = tp[:]
    for idx, val in enumerate(tp):
        prec[idx] = float(tp[idx]) / (fp[idx] + tp[idx])
    # 假设 tp = [1, 1, 2, 2]，fp = [0, 1, 1, 2]，则
    # prec = [1/(1+0), 1/(1+1), 2/(2+1), 2/(2+2)] = [1.0, 0.5, 0.667, 0.5]。
