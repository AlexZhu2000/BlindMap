...
from opencood.models.fuse_modules.zzh_where2comm_attn import ZZHWhere2comm
import torch

class ZZHPointPillarWhere2comm(nn.Module):
    def __init__(self, args):
        super(ZZHPointPillarWhere2comm, self).__init__()
        print('zzh pointpillarwhere2comm model....')
        # PIllar VFE
        ...
        self.fusion_net = ZZHWhere2comm(args['fusion_args'])
    ...
    def forward(self, data_dict):
        ...
        # print('psm_single: ', psm_single.shape)
        # print('rm_single: ', rm_single.shape)
        # spatial_features_2d:  torch.Size([2, 256, 100, 252])
        # spatial_features:  torch.Size([2, 64, 200, 504])
        # psm_single:  torch.Size([2, 2, 100, 252])
        # rm_single:  torch.Size([2, 14, 100, 252])
        if self.multi_scale:
        ...
        # fused_feature是ego车辆原本特征和infra侧经过communication_mask之后融合（max）的特征
        # print('fused_feature: ', fused_feature.shape)
        # fused_feature:  torch.Size([1, 256, 100, 252])
        psm = self.cls_head(fused_feature)
        rm = self.reg_head(fused_feature)
        # print('fused_feature psm: ', psm.shape)
        # print('fused_feature rm: ', rm.shape)
        # fused_feature psm:  torch.Size([1, 2, 100, 252])
        # fused_feature rm:  torch.Size([1, 14, 100, 252])
        output_dict = {'psm': psm,
                       'rm': rm
                       }
