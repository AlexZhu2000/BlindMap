...
class Communication(nn.Module):
    ...
    def forward(self, batch_confidence_maps, record_len, pairwise_t_matrix):
        ...
        for b in range(B):
            ...
            #  [2, 1, H, W] -->(sigmod) [2, 1, H, W] -->(max)[2, H, W] -->(unsqueeze)[2, 1, H, W]
            # max 操作实际上并没有在比较多个值中选择最大值,而是简单地"压缩"掉了大小为1的维度
            # ori_communication_maps 的最终形状是 [2, 1, H, W]，表示第 b 个批次中每个位置的最大置信度图，只有一个通道，表示最大置信度的值。
            if self.smooth:
            ...
            # 这里应该有问题，应该计算非ego车辆的communication_rate
