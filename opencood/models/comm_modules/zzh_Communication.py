...
class ZZHCommunication(nn.Module):
    def __init__(self, args):
        super(ZZHCommunication, self).__init__()
    ...
    def forward(self, batch_blind_maps_groups, record_len, pairwise_t_matrix):
        ...
        # batch_confidence_maps[0].shape: torch.Size([2, 1, H,W])
        # print('batch_size:', B)  B=6
        for b in range(B):
            ...
            # communication_mask_nodiag切片，将偶数行全部设置为 1
            communication_masks.append(communication_mask_nodiag)
            communication_rates.append(communication_rate)
            batch_communication_maps.append(infra_communication_maps*communication_mask_nodiag)
