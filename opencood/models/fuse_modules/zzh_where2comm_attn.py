...
from opencood.models.comm_modules.zzh_Communication import ZZHCommunication
from opencood.models.comm_modules.blindmap import BlindMap
import matplotlib.pyplot as plt
def visualize_blind_maps(batch_blind_maps, record_len):
    B = len(record_len)
    print('B:', B)
...
# 使用示例
# batch_blind_maps: [12, 1, 100, 252]
# record_len: [N1, N2, N3, N4, N5, N6]
...
class ZZHWhere2comm(nn.Module):
    def __init__(self, args):
        super(ZZHWhere2comm, self).__init__()
        print(f'***************{self.__class__.__name__}****************')
        self.communication = False
        self.round = 1
        if 'communication' in args:
            self.communication = True
            self.BlindMap = BlindMap(args['communication'])
            self.naive_communication = ZZHCommunication(args['communication'])
    ...
    def forward(self, x, rm, record_len, pairwise_t_matrix, backbone=None, heads=None):
        ...
        # print(f'*******************where2comm forward*****************self.multi_scale: {self.multi_scale}')
        # (B,L,L,2,3)
        ...
        if self.multi_scale:
            ...
            for i in range(self.num_levels):
                ...
                if i==0:
                    if self.communication:
                        # print(f'******************{__file__}***************')
                        batch_blind_maps = self.BlindMap(x, record_len, pairwise_t_matrix)
                        # visualize_blind_maps(batch_blind_maps, record_len)
                        # print('batch_blind_maps.shape:', batch_blind_maps.shape)
                        # batch_blind_maps.shape: torch.Size([12, 1, 100, 252])
                        # batch_blind_maps = batch_blind_maps.squeeze(1)
                        # (sum(cav), 1, H, W)
                        batch_blind_maps_groups = self.regroup(batch_blind_maps, record_len)
                        batch_confidence_maps = self.regroup(rm, record_len)
                        # print('rm.shape', rm.shape)
                        # batch_blind_maps_groups为tuple类型
                        # print('batch_blind_maps_groups.len:', len(batch_blind_maps_groups))
                        # batch_blind_maps_groups[0].shape: torch.Size([2, 1, 100, 252])
                        _, communication_masks, communication_rates = self.naive_communication(batch_blind_maps_groups, record_len, pairwise_t_matrix)
                ...
                # print('communication_masks.shape:', communication_masks.shape)
                # communication_masks.shape: torch.Size([12, 1, 100, 252])
                # print('x aftercommunication_masks shape:', x.shape)
                ############ 3. Fusion ####################################
                x_fuse = []
                for b in range(B):
                    # number of valid agent
            ...
            for b in range(B):
                ...
                node_features = batch_node_features[b]
                # node_features torch.Size([2, 2, 100, 252]
                if self.communication:
