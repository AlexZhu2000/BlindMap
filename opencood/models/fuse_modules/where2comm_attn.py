...
def visualize_blind_maps(batch_blind_maps, record_len):
    '''
    batch_blind_maps: tuple'''
    B = len(record_len)
    # print('B:', B)
     # 处理tuple类型
    if isinstance(batch_blind_maps, tuple):
       batch_blind_maps = torch.cat(batch_blind_maps, dim=0)
    batch_min = batch_blind_maps.min()
    batch_max = batch_blind_maps.max()
    batch_blind_maps = (batch_blind_maps - batch_min) / (batch_max - batch_min)
            ...
            for i in range(self.num_levels):
                ...
                if i==0:
                    if self.communication:
                        ...
                        # visualize_blind_maps(batch_confidence_maps, record_len)
