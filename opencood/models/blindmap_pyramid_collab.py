...
class BlindmapPyramidCollab(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        for modality_name in self.modality_name_list:
            ...
            #camera: encoder_m2 output shape: torch.Size([16, 128, 256, 512])
            #LiDAR:  encoder_m1 output shape: torch.Size([4, 64, 256, 512])
            feature = eval(f"self.backbone_{modality_name}")({"spatial_features": feature})['spatial_features_2d']
            # print(f"encoder_{modality_name} output shape: {feature.shape}")
            # encoder_m1 output shape: torch.Size([4, 64, 128, 256])
        ...
        # fused_feature shape: torch.Size([1, 384, 128, 256])
        if self.shrink_flag:
