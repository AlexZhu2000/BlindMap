...
- multiple collaborative perception methods
  ...
  - [x] [BlindMap [ICLR2024]](https://openreview.net/forum?id=KkrDUGIASk)
...
These training and testing instructions apply to all end-to-end training methods. Note that BlindMap requires that a collaborative base be constructed before aligning other agent types, see the next section for training for BlindMap. If you want to train a collaborative perception model based on the Pyramid Fusion, the following approach still applies.
...
BlindMap will first train a collaboration base and then align new agent type to this base. Follows our paper, we select LiDAR w/ PointPillars as our collaboration base.
### Step 1: Train the Collaboration Base
Suppose you are now in the `BlindMap/` folder. If this is your first training attempt, execute `mkdir opencood/logs`. Then 

```bash
mkdir opencood/logs/BlindMap_m1_based
mkdir opencood/logs/BlindMap_m1_based/stage1
mkdir opencood/logs/BlindMap_m1_based/stage1/m1_base

cp opencood/hypes_yaml/opv2v/MoreModality/BlindMap/stage1/m1_pyramid.yaml opencood/logs/BlindMap_m1_based/stage1/m1_base/config.yaml
python opencood/tools/train.py -y None --model_dir opencood/logs/BlindMap_m1_based/stage1/m1_base # you can also use DDP training
```
...
```bash
mkdir opencood/logs/BlindMap_m1_based/stage2
mkdir opencood/logs/BlindMap_m1_based/stage2/m2_alignto_m1
mkdir opencood/logs/BlindMap_m1_based/stage2/m3_alignto_m1
mkdir opencood/logs/BlindMap_m1_based/stage2/m4_alignto_m1

cp opencood/logs/BlindMap_m1_based/stage1/m1_base/net_epoch_bestval_at23.pth opencood/logs/BlindMap_m1_based/stage2/net_epoch1.pth # your bestval checkpoint!

ln -s opencood/logs/BlindMap_m1_based/stage2/net_epoch1.pth opencood/logs/BlindMap_m1_based/stage2/m2_alignto_m1
ln -s opencood/logs/BlindMap_m1_based/stage2/net_epoch1.pth opencood/logs/BlindMap_m1_based/stage2/m3_alignto_m1
ln -s opencood/logs/BlindMap_m1_based/stage2/net_epoch1.pth opencood/logs/BlindMap_m1_based/stage2/m4_alignto_m1

cp opencood/hypes_yaml/opv2v/MoreModality/BlindMap/stage2/m2_single_pyramid.yaml opencood/logs/BlindMap_m1_based/stage2/m2_alignto_m1/config.yaml
cp opencood/hypes_yaml/opv2v/MoreModality/BlindMap/stage2/m3_single_pyramid.yaml opencood/logs/BlindMap_m1_based/stage2/m3_alignto_m1/config.yaml
cp opencood/hypes_yaml/opv2v/MoreModality/BlindMap/stage2/m4_single_pyramid.yaml opencood/logs/BlindMap_m1_based/stage2/m4_alignto_m1/config.yaml
```

Then you can train new agent type without collaboration. These models can be trained in parallel.
```bash
python opencood/tools/train.py -y None --model_dir opencood/logs/BlindMap_m1_based/stage2/m2_alignto_m1 # you can also use DDP training
python opencood/tools/train.py -y None --model_dir opencood/logs/BlindMap_m1_based/stage2/m3_alignto_m1
python opencood/tools/train.py -y None --model_dir opencood/logs/BlindMap_m1_based/stage2/m4_alignto_m1
```

### Step 3: Combine and Infer
```bash
mkdir opencood/logs/BlindMap_m1_based/final_infer/ # create a log folder for final infer.

cp opencood/hypes_yaml/opv2v/MoreModality/BlindMap/final_infer/m1m2m3m4.yaml opencood/logs/BlindMap_m1_based/final_infer/config.yaml 

python opencood/tools/BlindMap_tools.py merge_final \
  opencood/logs/BlindMap_m1_based/stage2/m2_alignto_m1 \
  opencood/logs/BlindMap_m1_based/stage2/m3_alignto_m1 \
  opencood/logs/BlindMap_m1_based/stage2/m4_alignto_m1 \
  opencood/logs/BlindMap_m1_based/stage1/m1_base \
  opencood/logs/BlindMap_m1_based/final_infer
```
`python opencood/tools/BlindMap_tools.py merge_final` will automatically search the best checkpoints for each folder and merge them together. The collaboration base's folder (m1 here) should be put in the second to last place, while the output folder should be put last.

To validate the BlindMap's performance in open heterogeneous setting, i.e., gradually adding new agent types into the scene, we use `opencood/tools/inference_heter_in_order.py`.

```bash
python opencood/tools/inference_heter_in_order.py --model_dir opencood/logs/BlindMap_m1_based/final_infer 
```
...
We store our checkpoints files in [BlindMap's Huggingface Hub](https://huggingface.co/yifanlu/BlindMap/tree/main).
...
If you want to compare with BlindMap's model and you use spconv 1.2.1, you can still load from the checkpoint. To develop your model, please do not use these checkpoints.
